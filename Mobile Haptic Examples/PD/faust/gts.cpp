...
inline void PdUI3::add_elem(ui_elem_type_t type, const char *label)
{
...
inline void PdUI3::add_elem(ui_elem_type_t type, const char *label, float *zone)
{
  ui_elem_t *elems1 = (ui_elem_t*)realloc(elems, (nelems+1)*sizeof(ui_elem_t));
  if (elems1)
    elems = elems1;
...
void PdUI3::addButton(const char* label, float* zone)
{ add_elem(UI_BUTTON, label, zone); }
void PdUI3::addCheckButton(const char* label, float* zone)
{ add_elem(UI_CHECK_BUTTON, label, zone); }
void PdUI3::addVerticalSlider(const char* label, float* zone, float init, float min, float max, float step)
{ add_elem(UI_V_SLIDER, label, zone, init, min, max, step); }
void PdUI3::addHorizontalSlider(const char* label, float* zone, float init, float min, float max, float step)
{ add_elem(UI_H_SLIDER, label, zone, init, min, max, step); }
void PdUI3::addNumEntry(const char* label, float* zone, float init, float min, float max, float step)
{ add_elem(UI_NUM_ENTRY, label, zone, init, min, max, step); }

void PdUI3::addHorizontalBargraph(const char* label, float* zone, float min, float max)
{ add_elem(UI_H_BARGRAPH, label, zone, min, max); }
void PdUI3::addVerticalBargraph(const char* label, float* zone, float min, float max)
{ add_elem(UI_V_BARGRAPH, label, zone, min, max); }

void PdUI3::openTabBox(const char* label)
{
  if (!path.empty()) path += "/";
  path += mangle(label);
}
void PdUI3::openHorizontalBox(const char* label)
{
  if (!path.empty()) path += "/";
  path += mangle(label);
}
void PdUI3::openVerticalBox(const char* label)
{
  if (!path.empty()) path += "/";
  path += mangle(label);
}
...
#ifndef FAUSTCLASS 
#define FAUSTCLASS gts
#endif

class gts : public dsp {
  private:
  public:
	...
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		for (int i=0; i<count; i++) {
			output0[i] = (FAUSTFLOAT)((float)input0[i] > 0.0f);
		}
	}
};
...
static void *faust_new(t_symbol *s, int argc, t_atom *argv)
{
  ...
  x->inputs = x->outputs = x->buf = NULL;
    x->label = new std::string(sym(gts) "~");
  x->dsp = new gts();
  x->ui = new PdUI3(id?id->s_name:NULL);
...
extern "C" void faust_setup(gts)
{
  t_symbol *s = gensym(sym(gts) "~");
  faust_class =
  ...
  gts dsp = gts();
  post("[faust] %s: %d inputs, %d outputs", sym(gts) "~",
